...
//   음식 정보를 담을 DB 선언
   public static ArrayList<TFood> foods = new ArrayList<>();
      
//   음식 추가
   public void add(TFood food) {
      foods.add(food);
   }
...
//   예)한식 입력 시 한식만 조회
   public ArrayList<TFood> getFoods(String kinds){
      ArrayList<TFood> results = new ArrayList<>();
      for (TFood food : foods) {
...
//   예)한식 -> 중식, 음식 가격은 10% 상승
   public void updateKinds(TFood food) {
      for (TFood temp : foods) {
